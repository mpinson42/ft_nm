...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/30 14:47:37 by mpinson           #+#    #+#             */
/*   Updated: 2018/05/30 14:47:39 by mpinson          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		**sort_name_revers(struct nlist_64 *array,
...
int			print_output_revers(int nsyms, int sysmoff, int stroff, t_gen *g)
{
	char			*stringtable;
	...
	if (!(str = sort_name_revers(array, nsyms, stringtable, g)))
		return (-1);
	g->i = -1;
	while (++g->i < nsyms)
	{
		...
		while (++g->y < nsyms)
		{
			...
			if (ft_strcmp(str[g->i], stringtable + array[g->y].n_un.n_strx) == 0)
				print_nm(revers_uint64(array[g->y].n_value),
					revers_uint8(array[g->y].n_type),
					stringtable + array[g->y].n_un.n_strx, g);
		}
	}
	free(str);
	return (0);
...
int			bin_64_revers(t_gen *g)
{
	...
	g->ncmds = revers_uint32(g->header_64->ncmds);
	g->i = 0;
	...
	while (g->i < g->ncmds)
	{
		if (revers_uint32(g->lc->cmd) == LC_SYMTAB)
		{
			...
			if (print_output_revers(revers_uint32(g->sym->nsyms),
